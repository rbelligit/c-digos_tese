/*
 * mex_calc_exit_comp_bit_exit.cpp
 *
 *  Created on: 12 de mai de 2019
 *      Author: belli
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	std::cerr << "aqui8 ...." << std::endl;

	dataInp.sigma = mxGetScalar(prhs[5]);

	std::cerr << "aqui9 ...." << std::endl;
	...
	std::cerr << "aqui10 ...." << std::endl;

	dataInp.nBits = floor(log2(static_cast<double>(nMaps))+0.5);

	std::cerr << "aqui11 ...." << std::endl;

	calc_curv_exit_bits_to_check_and_check_to_bits(&dataInp, &dataOut);

	int ntt = dataOut.j_bits_to_check.size();
	...
	for(int i=0;i<ntt;++i)
	{
		ptr0[i] = dataOut.j_bits_to_check[i];
		ptr1[i] = dataOut.j_checks_to_bits[i];
	}
}
