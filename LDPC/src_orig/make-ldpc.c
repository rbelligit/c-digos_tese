...
)
{
	...
	if (no4cycle)
	{
		int numberOfOneInRow[M];
		int numberOfOneInCol[N];
#ifndef NEW_METHOD
		int firstC = 0;
#endif
		elim4 = 0;
		...
		//// Pega a densidade de 1 por linha inicial
		t = 0;
		for(j=0;j<M;++j)
		{
			numberOfOneInRow[j] = 0;
			for(f = mod2sparse_first_in_row(H,j);
					!mod2sparse_at_end(f);
		...
		for(j=0;j<N;++j)
		{
			numberOfOneInCol[j] = 0;
			for(f = mod2sparse_first_in_col(H,j);
					!mod2sparse_at_end(f);
					f=mod2sparse_next_in_col(f))
			{
				++numberOfOneInCol[j];
				printf("c(%d) r(%d) - ", f->col, f->row);
			}
			printf("\n");
		}
		...
		{
			printf("K=%d\n", k);
			k = 0; // número de ciclos encontrados.
			for (j = 0; j<N; j++) // for nas colunas....
			{
				printf("t=%d - j=%d - elim4=%d\n", t, j, elim4);
				for (e = mod2sparse_first_in_col(H,j); // FOR nos '1' da coluna j
						!mod2sparse_at_end(e);
						e = mod2sparse_next_in_col(e))
				{
					for (f = mod2sparse_first_in_row(H,mod2sparse_row(e)); // FOR nos '1' da linha 'e' (FOR atrás)
							!mod2sparse_at_end(f);
							f = mod2sparse_next_in_row(f))
					{
						if (f==e) continue;
						for (g = mod2sparse_first_in_col(H,mod2sparse_col(f)); // FOR nos '1' da coluna de 'f'
								!mod2sparse_at_end(g);
								g = mod2sparse_next_in_col(g))
						{
							if (g==f) continue;
							for (h = mod2sparse_first_in_row(H,mod2sparse_row(g)); // FOR nos '1' da linha de
									!mod2sparse_at_end(h);
									h = mod2sparse_next_in_row(h))
							{
								if (mod2sparse_col(h)==j)
								{
									...
									goto nextj;
#else
									// temos um ciclo de ordem 4, desconstruindo o circuito de ordem 4 aleatoriamente.
									k += 1;
									elim4 += 1;
									{
										int kk;
										mod2entry *change;
										mod2entry *tobeCHanged;
										int rowT, colT;
										int rowC, colC;
										...
										while(1)
										{
											do
											{
												i = rand_int(N);
												 ...
												 i == mod2sparse_row(f));
											int ii, jj;
											change = mod2sparse_first_in_col(H, i);
											if(change->col < 0 || change->row < 0)
											{
												printf("Estranho..... coluna %d\n", i);
												exit(0);
											}
											kk = rand_int(numberOfOneInCol[i]);
											//printf("kk=%d col=%d --> ii=%d - c(%d) r(%d) - ",kk, i, -1, change->col, change->row);
											for(ii=0;ii<kk;ii++)
											{
												change = mod2sparse_next_in_col(change);
												//printf("ii=%d - c(%d) r(%d) - ", ii, change->col, change->row);
											}
											//printf("\n");
											if(change->col < 0 || change->row < 0)
											{
												printf("Estranho..... coluna i=%d - kk=%d - N=%d - nCols=%d - c(%d) r(%d)\n", i, kk, N, numberOfOneInCol[i],
											...
											case 3:
												...
												break;
											default:
												printf("Erro.... rand value %d", jj);
												exit(0);
												break;
											}
											rowT = mod2sparse_row(tobeCHanged);
											colT = mod2sparse_col(tobeCHanged);

											rowC = mod2sparse_row(change);
											colC = mod2sparse_col(change);

											if(mod2sparse_col(tobeCHanged) != mod2sparse_col(change) &&
											   mod2sparse_find(H, rowT, colC)==NULL &&
											   mod2sparse_find(H, rowC, colT)== NULL)
											{
												// precisa checar também se não tem
												break;
											}
											else
											{
...

										{
											...
											printf("Deletado - c(%d) r(%d) e c(%d) r(%d) - Adicionado c(%d) r(%d) e c(%d) r(%d)\n",
													colT, rowT, colC, rowC,
													colC, rowT, colT, rowC);
											mod2sparse_delete(H,change);
											mod2sparse_delete(H,tobeCHanged);

											mod2sparse_insert(H, rowT, colC);
											mod2sparse_insert(H, rowC, colT);
